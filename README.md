...
전부 **국내 모델**이다(대회 규정). 모델명은 코드에 적힌 실제 호출 문자열 그대로다.
provider는 `.env`로 바꿀 수 있지만 **판례 경로만은 예외로 `solar-pro2`가 하드코딩**돼 있다.
...
| 판례 — 공통점 문장 (`commonPoint`) | `solar-pro2` | 검색된 판례들의 공통점을 쓴다 — **모델이 자유 문장을 쓰는 유일한 자리** | 사건번호·출처는 서버가 원문에서 복사. 응답의 `case_id`가 검색 결과에 없으면 그 항목 폐기 |
| 판례 — 결과 분류 (`result`) | `solar-pro2` | **분류만 고른다** | 화면에 나가는 문구는 코드 상수(`OUTCOME_TEXT`). 자유 문장을 쓸 통로가 없다 — 승소 판례가 '안심 신호'로 읽히지 않게 |
| 판례 — 강조 구간 (`emphasis`) | `solar-pro2` | 굵게 볼 **부분 문자열**을 고른다 | 원문에 실재하는 문자열만 채택. 못 찾으면 굵기만 안 붙는다. 본문을 바꾸지 않는다 |
...
| 비교 검증 | `LGAI-EXAONE/K-EXAONE-236B-A23B`<br/>FriendliAI 경유 | **설명 생성 모델 선정을 위한 비교 검증에만 썼다**(2026-08-14 실측 18회). 서비스 응답 생성에는 쓰지 않는다 | 실사용 경로에서 호출되지 않는다 — 환경변수 스위치가 꺼져 있다 |

> **`advice`(판례 조언) · `termGlossary`(용어 툴팁) · 판례 태깅에는 LLM이 없다.**
> `advice`는 100% 사람 큐레이션이고 LLM 입력에도 들어가지 않는다. 태깅은 사람(11건)과
> 키워드 규칙(29건)이 나눠 하고, 편입 게이트도 규칙이다. 이것은 약점이 아니라
> **사람이 검수한 문장은 모델이 못 건드린다**는 설계다.
...
| 이름 | 역할 |
|---|---|
| **이영호** | 팀대표 · LLM · RAG (판례 검색 파이프라인) |
| **남규혁** | 메인 개발 (앱·서버 전반, 규칙 엔진, 가드레일 설계) |
| **정민재** | 법률 큐레이션 · 판례 검수 |
| **최가은** | 현장 인터뷰 · 검증 |
| **김시원** | 경쟁 분석 · 페르소나 |
