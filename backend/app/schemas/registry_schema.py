...
REGISTRY_JSON_SCHEMA: dict[str, Any] = {
    ...
    "properties": {
        ...
        # 부동산고유번호 — **같은 집인지 가리는 유일한 확정 근거**(2026-08-14 대조 기능).
        # 주소는 표기가 흔들린다(도로명/지번, '서울'/'서울특별시', 동호수 유무). 두 등기부가
        # 같은 집인지 숫자 하나로 판별할 수 있는 것은 이 번호뿐이라, 대조(compare)는 이 값이
        # 양쪽에 있으면 이것만 본다. 못 읽으면 주소로 견주되 애매하면 '확인 못 함'으로 남긴다.
        "unique_number": {
            "type": "string",
            "description": (
                "[공통] 문서 상단의 부동산고유번호. 하이픈 포함해 보이는 그대로. "
                "예: '1355-1996-123456'. 없으면 생략"
            ),
        },
        ...
        # ⚠ 아래 두 필드는 **서로 배타적이어야 한다** (2026-07-28 실측 대응).
        #
        # 무엇이 있었나: 예전 설명은 "[갑구] 가압류 목록" / "[갑구] 압류 목록" 뿐이었다.
        # 그런데 '가압류'는 글자에 '압류'를 품고 있어서, 배타 조건이 없으면 모델이
        # 가압류 항목을 **양쪽에 다 넣는다.** 실측(run 20260728_225552_594): 종이 원문이
        # 가압류인 순위 2·7·8이 `seizures`와 `provisional_seizures`에 동시에 실려
        # 압류가 4건으로 보였다(참값 1건). 5회 반복해도 같았으므로 흔들림이 아니다.
        #
        # 왜 이 문장이 근거가 있나: **같은 문서·같은 사진**인데 두 번째 경로(Solar 구조화)는
        # 압류 1건으로 맞혔다. 그 프롬프트(`llm/prompts.py`)에는 "등기목적이 '압류'이면
        # seizures, '가압류'이면 provisional_seizures"라는 **배타 조건이 이미 있었다.**
        # 즉 모델 능력 차이가 아니라 **우리가 한쪽에만 조건을 안 준 것**이다.
        #
        # ⚠ 이 문구는 **추출 지시**일 뿐 판정 기준이 아니다. 임계값·가중치와 무관하며
        #   등급 계산에는 닿지 않는다(회귀표 `tests/test_verdict_regression.py`가 봉인).
        "provisional_seizures": _claim_like_entry(
            "[갑구] 가압류 목록(각 항목별 말소 여부 포함). "
            "등기목적이 '가압류'인 항목만 넣는다. 등기목적이 '압류'인 항목은 넣지 않는다"
        ),
        "provisional_dispositions": _claim_like_entry(
            "[갑구] 가처분 목록(각 항목별 말소 여부 포함). "
            "등기목적이 '가처분'인 항목만 넣는다"
        ),
        "seizures": _claim_like_entry(
            "[갑구] 압류 목록(각 항목별 말소 여부 포함). "
            "등기목적이 정확히 '압류'인 항목만 넣는다. "
            "'가압류'는 제외한다 — 가압류는 provisional_seizures 에만 넣고 "
            "seizures 에는 절대 넣지 않는다. "
            "한 항목이 seizures 와 provisional_seizures 양쪽에 동시에 들어가서는 안 된다"
        ),
