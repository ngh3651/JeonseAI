...
9. emphasis는 **화면에서 굵게 보여줄 구절**을 고르는 칸입니다. 새 문장을 쓰는 칸이 아닙니다.
   - "common_point" 키에는 당신이 쓴 common_point 문장에서 **그대로 잘라낸** 구절을 넣으세요.
   - "result" 키에는 입력의 '결과' 문장에서 **그대로 잘라낸** 구절을 넣으세요.
     입력에 '결과'가 없으면 "result" 키를 아예 빼세요.
   - "advice" 키에는 입력의 '조언' 문장에서 **그대로 잘라낸** 구절을 넣으세요.
     입력에 '조언'이 없으면 "advice" 키를 아예 빼세요.
     ※ '결과'와 '조언'은 사람이 검수해 확정한 문장입니다. **고쳐 쓰지 말고, 다른 필드에
       옮겨 쓰지도 마세요.** 여기서 할 일은 그 문장의 일부를 가리키는 것뿐입니다.
   - 한 글자라도 다르면 그 필드는 버려집니다. 요약하거나 다듬지 말고 **복사해서** 넣으세요.
   - 필드당 최대 2개, 하나당 30자 이내, 합쳐서 그 문장의 30%를 넘지 마세요.
   - 고를 것: 보증금을 잃었다는 사실, 금액·비율·기간, 그 사건의 결정적 조건.
   - 고르지 말 것: 문장 전체, 조사·접속사만 있는 토막.

다음 JSON 형식으로만 응답하세요 (다른 텍스트 금지):
{"cases": [{"case_id": "입력의 case_id 그대로", "easy_summary": "...", "common_point": "...", "outcome_kind": "lost", "emphasis": {"common_point": ["..."], "result": ["..."]}}]}
        ...
        "판례_목록": [
            {
                ...
                "결과": m.doc.outcome,
                # 2026-08-14 D23: 조언은 **강조 구간을 고르라고** 넣는다(읽기 전용).
                # 출력 모델에 advice 필드가 없어 되돌려 받을 통로가 없고, emphasis로
                # 돌아온 값도 부분 문자열이 아니면 폐기된다 — 가리킬 수만 있다.
                "조언": m.doc.advice,
