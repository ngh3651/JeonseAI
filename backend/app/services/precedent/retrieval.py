...
BM25_K1 = 1.5   # BM25 표준 기본값(Robertson k1 1.2~2.0 권장 구간) — 평가셋으로만 조정
BM25_B = 0.75   # BM25 표준 기본값(길이 정규화) — 평가셋으로만 조정

# 노출 게이트 하한선 — 백엔드별 코사인 분포가 달라 서명 접두어로 구분한다.
# (upstage 4096차원 실측: 관련 쌍 0.55~0.75 / 무관 쌍 0.3대 — 평가셋 캘리브레이션 값)
VECTOR_SIM_FLOOR = {
    "upstage": 0.45,
    # ⚠ local 값은 upstage 실측의 복사 — KURE 등 로컬 백엔드 전환 시(대량 배치 후)
    # 해당 모델의 유사도 분포로 평가셋 재캘리브레이션 필수 (rule-auditor 재감사 2026-07-22)
    "local": 0.45,
