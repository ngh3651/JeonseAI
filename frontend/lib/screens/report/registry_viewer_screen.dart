...
/// 서버가 보낸 "찾아본 것" 중 **화면 하단 회색 줄에 올릴 문장**을 골라내는 표식.
///
/// 문장 자체는 서버가 만든다. 앱은 고르기만 한다 — 새 사유가 생겨도 이 목록만 늘린다.
///
/// 2026-07-28: 두 종류를 담게 됐다.
/// ⑴ **표시하지 못한 것** — 원래 목적(개수가 안 맞는 이유를 대는 자리)
/// ⑵ **두 방법으로 읽었다는 사실** — 교차검증. 이걸 빼면 화면에 **불일치 경고만 남고
///    "확인했다"는 절반이 사라져** "우리도 모르겠다"로 읽힌다(2026-07-28 페르소나 2인 지적).
///
/// ⚠ 문자열 매칭이라 **서버가 문구를 바꾸면 조용히 사라진다.** 실제로 오늘 새로 만든
///   고지 3종이 전부 여기서 탈락하고 있었다. 백엔드 문구를 고칠 때는 이 목록을 함께 본다.
///   (근본 해결은 서버가 분류 태그를 실어 보내는 것 — api-contract 갱신이 필요해 후속으로 뒀다.)
const List<String> kUnmarkedNoteMarkers = [
  '표시하지 않았',
  '찾지 못했',
  '생략했',
  '교차 확인', // 두 방법이 일치했다는 신뢰 문장
  '두 방법으로 읽었더니', // 두 방법이 갈렸다는 고지
  '한 가지 방법으로만', // 두 번째 확인을 못 했다는 고지
  '자동으로 맞췄', // 사진 순서를 서버가 고쳤다는 고지
  '말할 수 없어요', // 못 본 쪽이 있어 '없다'고 단정할 수 없다는 고지
  '계산에서 제외했', // 말소로 확인해 위험 계산에서 뺐다는 고지 (2026-08-14 D10)
];
