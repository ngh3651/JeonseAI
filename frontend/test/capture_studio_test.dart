...
void main() {
  ...
  testWidgets('[촬영 시작] → 확인 화면의 주 버튼이 [다음 장 찍기]다', (tester) async {
    ...
    expect(find.text('1장째 찍었어요'), findsOneWidget);
    // 루프의 핵심 — 기본 동작이 "계속 찍기"다.
    expect(find.text('다음 장 찍기'), findsOneWidget);
  ...
  testWidgets('[다음 장 찍기]를 누르면 장수가 올라간다', (tester) async {
    ...
    await tester.tap(find.text('촬영 시작'));
    await tester.pumpAndSettle();
    await tester.tap(find.text('다음 장 찍기'));
    await tester.pumpAndSettle();
    expect(find.text('2장째 찍었어요'), findsOneWidget);
    expect(find.text('완료 · 2장'), findsOneWidget);
  });
  ...
  testWidgets('[완료]로 돌아오면 작업 모드가 되고 트레이에 사진이 있다', (tester) async {
    ...
    await tester.tap(find.text('촬영 시작'));
    await tester.pumpAndSettle();
    await tester.tap(find.text('완료 · 1장'));
    await tester.pumpAndSettle();
    ...
    final FilledButton button = tester.widget(
      find.ancestor(of: find.text('분석하기'), matching: find.byType(FilledButton)),
    );
  ...
  testWidgets('보증금은 28px 한글 표기 · 입력은 세 자리 쉼표 · 접미사는 작고 흐리게', (tester) async {
    ...
    // 값이 없을 때의 자리
    expect(find.text('얼마를 맡기시나요?'), findsOneWidget);

    await tester.enterText(find.byType(TextField).first, '12000');
    await tester.pumpAndSettle();

    // 한글 표기가 28px로 크게 (이 화면에서 가장 큰 숫자)
    final Text won = tester.widget(find.text('1억 2,000만원'));
  ...
  testWidgets('키보드가 올라와도 [분석하기]가 화면 안에 있다', (tester) async {
    ...
    final double before = tester.getBottomLeft(find.text('분석하기')).dy;
    ...
    final double after = tester.getBottomLeft(find.text('분석하기')).dy;
  ...
  testWidgets('자동 조회값을 미리 받으면 출처 라벨이 뜬다', (tester) async {
    ...
    expect(find.text('자동 조회 · 공시가격 기준 (2025.1.1)'), findsOneWidget);
  });
  ...
  testWidgets('사용자가 자동 조회값을 고치면 라벨이 "직접 입력하신 값"으로 바뀐다', (tester) async {
    ...
    expect(find.text('직접 입력하신 값'), findsOneWidget);
    expect(find.text('자동 조회 · 공시가격 기준 (2025.1.1)'), findsNothing);
  });
